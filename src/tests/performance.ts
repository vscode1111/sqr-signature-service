...
const SRV_URL = 'https://web3.main.dev.msq.local/signature/api';
// const SRV_URL = 'https://web3.stage.msq.local/signature/api';
//0x4A6d75e590A2E511d6745d0a938A971c9B69B71C
//0x4a6d75e590a2e511d6745d0a938a971c9b69b71c

const httpsAgent = new https.Agent({ rejectUnauthorized: false });

async function getLaunchpadDepositSignature(): Promise<GetWEB3ProRataDepositSignatureResponse | null> {
  const userId = uuidv4();
  const transactionId = uuidv4();

  const requestBody: Omit<GetWEB3PaymentGatewayDepositSignatureParams, 'network'> = {
  ...
  const response = await axios.post<GetWEB3ProRataDepositSignatureResponse>(
